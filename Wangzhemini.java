...
// 游戏主类
 class GloryOfKingsMUD {
    ...
    private void practiceSkill() {
        ...
        while (true) {
            System.out.println("\n===== 技能练习场 =====");
            System.out.println("英雄: " + hero.getName() + " Lv." + hero.getLevel());
            System.out.println("生命值: " + hero.getHealth() + "/" + hero.getMaxHealth());
            System.out.println("攻击力: " + hero.getAttack() + " 防御力: " + hero.getDefense());
            ...
            int choice = input.getMenuChoice("选择练习模式", options);
    ...
    // 练习单个技能
    private void practiceSingleSkill() {
        Hero hero = currentPlayer.getHero();

        while (true) {
            System.out.println("\n===== 单个技能练习 =====");
            System.out.println("🎯 练习木桩生命值: 200");
            System.out.println("普通攻击伤害: " + hero.getAttack());

            List<String> skillOptions = new ArrayList<>();
            for (int i = 0; i < hero.getSkills().size(); i++) {
                Skill skill = hero.getSkills().get(i);
                String readyStatus = hero.isSkillReady(skill.getName()) ? "✅" : "⏰";
                skillOptions.add(readyStatus + " " + skill.getName() +
                        " - 伤害:" + skill.getDamage() +
                        " 冷却:" + skill.getCooldown() + "秒");
            }
            skillOptions.add("普通攻击");
            skillOptions.add("返回");

            int choice = input.getMenuChoice("选择要使用的技能（可反复练习）", skillOptions);

            if (choice <= hero.getSkills().size()) {
                // 使用技能
                Skill skill = hero.getSkills().get(choice - 1);
                practiceSingleSkillExecution(skill);

            } else if (choice == hero.getSkills().size() + 1) {
                // 普通攻击
                practiceNormalAttack();

            } else if (choice == hero.getSkills().size() + 2) {
                // 返回
                break;
            }
    ...
    // 执行单个技能练习
    private void practiceSingleSkillExecution(Skill skill) {
        Hero hero = currentPlayer.getHero();
        int dummyHealth = 200;

        System.out.println("\n💥【" + skill.getName() + "】练习开始！");
        System.out.println("📖 技能描述: " + skill.getDescription());

        // 计算伤害
        int baseDamage = skill.getDamage();
        int attackBonus = hero.getAttack() / 3; // 攻击力加成
        int randomBonus = random.nextInt(6); // 随机波动
        int totalDamage = baseDamage + attackBonus + randomBonus;

        // 技能释放动画
        System.out.print("释放技能中");
        for (int i = 0; i < 3; i++) {
            System.out.print("💫");
            try {
        ...
        // 显示伤害效果
        System.out.println("\n\n💥 技能命中练习木桩！");
        System.out.println("📊 伤害计算:");
        System.out.println("   基础伤害: " + baseDamage);
        System.out.println("   攻击加成: " + attackBonus);
        System.out.println("   随机波动: " + randomBonus);
        System.out.println("   ⚔️ 总伤害: " + totalDamage);

        // 更新木桩生命值
        dummyHealth -= totalDamage;
        System.out.println("\n🎯 练习木桩:");
        System.out.println("   受到伤害: " + totalDamage);
        System.out.println("   剩余生命: " + Math.max(0, dummyHealth));

        if (dummyHealth <= 0) {
            System.out.println("💀 练习木桩被摧毁！需要新的木桩...");
        }
        ...
        // 显示技能评价
        displaySkillRating(skill, totalDamage);
    }

    // 练习普通攻击
    private void practiceNormalAttack() {
        Hero hero = currentPlayer.getHero();
        int dummyHealth = 200;
        ...
        int randomBonus = random.nextInt(5) - 2; // -2到+2的随机波动
        int totalDamage = Math.max(1, baseDamage + randomBonus);

        // 攻击动画
        System.out.print("攻击中");
        for (int i = 0; i < 3; i++) {
            System.out.print("⚔️");
            try {
        ...
        System.out.println("\n\n⚔️ 命中练习木桩！");
        System.out.println("📊 伤害计算:");
        System.out.println("   基础攻击力: " + baseDamage);
        System.out.println("   随机波动: " + randomBonus);
        System.out.println("   ⚔️ 总伤害: " + totalDamage);
    ...
    // 练习技能连招
    private void practiceSkillCombo() {
        Hero hero = currentPlayer.getHero();

        System.out.println("\n===== 技能连招练习 =====");
        System.out.println("🎯 连招练习木桩生命值: 300");
        System.out.println("💡 提示：选择2-3个技能组成连招，造成最大伤害！");

        List<Skill> selectedSkills = new ArrayList<>();
        int maxComboLength = Math.min(3, hero.getSkills().size());

        while (selectedSkills.size() < maxComboLength) {
            System.out.println("\n当前连招 (" + selectedSkills.size() + "/" + maxComboLength + "):");
            for (int i = 0; i < selectedSkills.size(); i++) {
                System.out.println("  " + (i + 1) + ". " + selectedSkills.get(i).getName());
            }
            ...
            for (int i = 0; i < hero.getSkills().size(); i++) {
                Skill skill = hero.getSkills().get(i);
                // 检查技能是否已选择
                boolean alreadySelected = selectedSkills.stream()
                        .anyMatch(s -> s.getName().equals(skill.getName()));
                String status = alreadySelected ? "✅已选择" : "可选";
                skillOptions.add(status + " " + skill.getName() + " (伤害:" + skill.getDamage() + ")");
            }
            skillOptions.add("完成选择，开始连招");
            skillOptions.add("重新选择");
            skillOptions.add("取消连招练习");

            int choice = input.getMenuChoice("选择第" + (selectedSkills.size() + 1) + "个技能", skillOptions);

            if (choice <= hero.getSkills().size()) {
                Skill skill = hero.getSkills().get(choice - 1);

                // 检查是否已选择
                if (selectedSkills.stream().anyMatch(s -> s.getName().equals(skill.getName()))) {
                    System.out.println("⚠️ 此技能已在连招中，请选择其他技能！");
                    continue;
                }

                selectedSkills.add(skill);
                System.out.println("✅ 已添加: " + skill.getName());

            } else if (choice == hero.getSkills().size() + 1) {
                // 完成选择，开始连招
                if (selectedSkills.size() >= 2) {
                    executeComboPractice(selectedSkills);
                    break;
                } else {
                    System.out.println("⚠️ 至少需要选择2个技能才能形成连招！");
                }

            } else if (choice == hero.getSkills().size() + 2) {
                // 重新选择
                selectedSkills.clear();
                System.out.println("🔄 连招已重置，请重新选择技能...");

            } else if (choice == hero.getSkills().size() + 3) {
                // 取消
                System.out.println("❌ 取消连招练习");
                return;
            }
        }
    ...
    // 执行连招练习
    private void executeComboPractice(List<Skill> comboSkills) {
        Hero hero = currentPlayer.getHero();
        int comboDummyHealth = 300;
        int totalDamage = 0;

        System.out.println("\n💥【连招练习】开始！");
        System.out.println("连招顺序: ");
        for (int i = 0; i < comboSkills.size(); i++) {
            System.out.println("  " + (i + 1) + ". " + comboSkills.get(i).getName());
        }
        System.out.println("\n🎯 目标木桩生命值: " + comboDummyHealth);

        // 执行连招
        for (int i = 0; i < comboSkills.size(); i++) {
            Skill skill = comboSkills.get(i);
            ...
            // 计算单技能伤害
            int baseDamage = skill.getDamage();
            int attackBonus = hero.getAttack() / 4;
            int comboBonus = (i + 1) * 2; // 连招位置加成
            int skillDamage = baseDamage + attackBonus + comboBonus;
            ...
            System.out.println("\n⚔️ 造成伤害: " + skillDamage);
            System.out.println("   (基础:" + baseDamage + " + 攻击加成:" + attackBonus + " + 连招加成:" + comboBonus + ")");

            totalDamage += skillDamage;
            comboDummyHealth -= skillDamage;
        ...
        System.out.println("🎯 木桩剩余生命值: " + Math.max(0, comboDummyHealth));

        // 连招评价
        System.out.println("\n📊 连招评价:");
        if (totalDamage >= 250) {
            System.out.println("🏆 完美连招！伤害爆表！");
            System.out.println("💪 你已经掌握了" + hero.getName() + "的精髓连招！");
        } else if (totalDamage >= 180) {
            System.out.println("✨ 优秀连招！");
            System.out.println("👍 技能衔接流畅，伤害很高！");
        } else if (totalDamage >= 120) {
            System.out.println("✅ 良好连招！");
            System.out.println("💡 可以尝试调整技能顺序或增加连招段数");
        } else {
            System.out.println("📈 基础连招！");
            System.out.println("🎯 建议多练习单个技能，提高技能熟练度");
        }

        // 连招建议
        if (comboDummyHealth > 150) {
            System.out.println("\n💡 建议：可以尝试更高伤害的技能组合！");
        }
    }

    // 显示技能评价
    private void displaySkillRating(Skill skill, int damage) {
        System.out.println("\n📊 技能评价:");

        if (damage >= 30) {
            System.out.println("🏆 超高爆发！");
            System.out.println("💥 这个技能可以造成毁灭性伤害！");
        } else if (damage >= 20) {
            System.out.println("✨ 强力技能！");
            System.out.println("👍 在对战中可以显著削弱敌人！");
        } else if (damage >= 10) {
            System.out.println("✅ 均衡技能！");
            System.out.println("⚖️ 伤害与冷却时间平衡良好");
        } else {
            System.out.println("🛡️ 辅助/控制技能");
            System.out.println("🌀 主要作用是控制敌人或提供增益效果");
        }
        ...
        switch (skill.getName()) {
            ...
            case "灼日之矢":
            case "狂热弹幕":
                System.out.println("  这是大招，冷却时间较长，建议在关键时刻使用");
                break;
            case "失心":
            case "火球术":
            case "河豚手雷":
            case "多重箭矢":
            case "豪气斩":
                System.out.println("  基础技能，冷却时间短，适合频繁使用");
                break;
            case "偶像魅力":
            case "幻纱之灵":
            case "惩戒射击":
            case "静默之箭":
                System.out.println("  带有控制效果的技能，适合先手或打断敌人");
                break;
            case "流云步":
            case "漫游之枪":
            case "正义潜能":
                System.out.println("  生存/位移技能，用于保命或调整位置");
                break;
        }
    }
